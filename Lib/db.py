...
class DB():
	...
	def create_radiotheaters_table(self):
		sql = """
			CREATE TABLE IF NOT EXISTS car_search(
				...
				CONSTRAINT title_date UNIQUE (title, Car_year, Engin_type, Price, Аccumulated_km)
			);
		"""
	...
	def insert_row(self, row_data):
		sql = """
			INSERT IGNORE INTO car_search
				() title, Car_year, Engin_type, Price, Аccumulated_km)
				VALUES ( %s, %s, %s, %s, %s)
		"""
	...
	def select_all_data(self):
		sql = "SELECT id, title, Car_year, Engin_type, Price, Аccumulated_km FROM  car_search"
	...
	def get_column_names(self):
		sql = "SELECT id, title, Car_year, Engin_type, Price, Аccumulated_km  FROM  car_search LIMIT 1;"
